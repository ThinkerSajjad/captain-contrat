...
import { llcQuestions } from "@/data/llcQuestions";
    ...
    <section className="py-16 bg-white" id="how-to-create">
      <PageContainer>
        <h2 className="text-3xl lg:text-5xl font-bold text-slate-800 mb-8 text-center">
          Understand everything about creating an LLC online
        </h2>
          ...
          <div className="order-2 lg:order-1 lg:w-1/3">
            <div className="bg-white p-6 rounded-lg border border-blue-500 sticky top-24">
              ...
              <ul className="space-y-2 text-sm">
                {llcQuestions.sections.map((section, index) => (
                  <li key={section.id}>
                    <button 
            ...
            <div className="bg-white p-4 rounded-lg mb-8 border border-l-4 border-l-amber-500 border-gray-100">
              <div className="flex items-center mb-4">
                <div className="w-12 h-12 rounded-full overflow-hidden mr-4">
                  <Image
                    src={llcQuestions.author.image}
                    alt={`${llcQuestions.author.name}, ${llcQuestions.author.title}`}
                    width={48}
                    height={48}
                ...
                <div>
                  <p className="font-semibold text-xl lg:text-2xl">Written by {llcQuestions.author.name}, {llcQuestions.author.title}</p>
                  <p className="text-gray-500 lg:text-sm text-xs">Updated: {llcQuestions.author.date}</p>
                </div>
              </div>
              <p className="text-sm lg:text-base text-gray-600">
                {llcQuestions.author.bio}
              </p>
            </div>

            {/* Content sections */}
            <div className="space-y-12">
              {llcQuestions.sections.map((section: Section) => (
