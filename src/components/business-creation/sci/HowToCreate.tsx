...
import { sciQuestions } from "@/data/sciQuestions";
    ...
    <section className="py-16 bg-white" id="how-to-create">
      <PageContainer>
        <h2 className="text-3xl lg:text-5xl font-bold text-slate-800 mb-8 text-center">
          Understand everything about creating an SCI online
        </h2>
          ...
          <div className="order-2 lg:order-1 lg:w-1/3">
            <div className="bg-white p-6 rounded-lg border border-blue-500 sticky top-24">
              ...
              <ul className="space-y-2 text-sm">
                {sciQuestions.sections.map((section, index) => (
                  <li key={section.id}>
                    <button 
            ...
            <div className="bg-white p-4 rounded-lg mb-8 border border-l-4 border-l-amber-500 border-gray-100">
              <div className="flex items-center mb-4">
                <div className="w-12 h-12 rounded-full overflow-hidden mr-4">
                  <Image
                    src={sciQuestions.author.image}
                    alt={`${sciQuestions.author.name}, ${sciQuestions.author.title}`}
                    width={48}
                    height={48}
                ...
                <div>
                  <p className="font-semibold text-xl lg:text-2xl">Written by {sciQuestions.author.name}, {sciQuestions.author.title}</p>
                  <p className="text-gray-500 lg:text-sm text-xs">Updated: {sciQuestions.author.date}</p>
                </div>
              </div>
              <p className="text-sm lg:text-base text-gray-600">
                {sciQuestions.author.bio}
              </p>
            </div>

            {/* Content sections */}
            <div className="space-y-12">
              {sciQuestions.sections.map((section: Section) => (
                <div key={section.id} id={section.id} className="scroll-mt-24">
                  ...
                  {section.steps && (
                    <ol className="list-decimal pl-5 space-y-6 text-gray-700">
                      {section.steps.map((step: Step) => (
                        <li key={step.number}>
                          <h4 className="font-semibold text-slate-700 mb-2">{step.number}. {step.title}</h4>
                  ...
                  {section.additionalContent && !section.additionalList && typeof section.additionalContent === 'string' && (
